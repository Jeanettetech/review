...
//        final var pendingState = state.getChildOrders().stream().filter(order->order.getState()==OrderState.PENDING).count();
...
//        final var filledQuantity = state.getChildOrders().stream().filter(order -> order.getFilledQuantity() > 0).count();
                ...
                final double bidAskSpreadThreshold = 20;//normally less than 1% from research based on this order book set to 4.5
...
//            if (state.getChildOrders().size() < 2) {//this line works for unit test on version one
                    ...
                    String volumeImbalanceOutput = calculateVolumeImbalance(state);
            ...
            protected String calculateVolumeImbalance(SimpleAlgoState state) {
                final AskLevel ask = state.getAskAt(0);
                final BidLevel bid = state.getBidAt(0);

                double volumeImbalance = (double) (bid.quantity - ask.quantity) / (bid.quantity + ask.quantity);
            ...
            private void childOrderLog(SimpleAlgoState state) {
                ...
                logger.info("[MYALGO] Child Order Log Count: Filled=" + filledState.size() + " ,Pending=" + pendingState.size() + " ,Cancelled=" + cancelledState.size());
            }

            private void logBestPrices(BidLevel bid, AskLevel ask) {
